...
% Accuracy for Rimone3 FRFCM results

Traindata = xlsread('RimoneCDRTrain.xlsx');

Testdata = xlsread('RimoneCDRTest.xlsx');

TrainLabel = xlsread('RimoneLabelTrain.xlsx');

TestLabel = xlsread('RimoneLabelTest.xlsx');


%SVM1 classifier
tic;
 md1 = fitcsvm(Traindata,TrainLabel);
 
 [YPred, scores]= predict( md1,Testdata);
 
 accuracy = mean(YPred ==TestLabel);
 
 %figure, plotconfusion(TestLabel,YPred)
 CM = confusionmat(YPred,TestLabel);
 AR = (trace(CM)./37)*100;
 %%
 
 % RImone Cup test
 Traindata = xlsread('RimonecupTrain.xlsx');

Testdata = xlsread('RimonecupTest.xlsx');
...
train_data = [A(1:56,1:4);A(92:end,1:4)];
train_class = [A(1:56,5);A(92:end,5)];
test_data = [A(57:91,1:4)];
test_class = [A(57:91,5)];

Mdl =fitcsvm(train_data, train_class);

Tested = predict(Mdl, test_data); 

CM = confusionmat(test_class,Tested);
